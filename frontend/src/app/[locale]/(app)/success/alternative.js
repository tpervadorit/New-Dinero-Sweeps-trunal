...
const SSOSuccessAlternative = () => {
  ...
  useEffect(() => {
    ...
    if (error) {
      ...
      setStatus('error');
      setTimeout(() => {
        handleRedirect('/?error=sso_failed');
      }, 2000);
      return;
    }
    ...
    if (token) {
      try {
        ...
        setStatus('error');
        setTimeout(() => {
          handleRedirect('/?error=token_invalid');
        }, 2000);
      }
    } else {
...
export default SSOSuccessAlternative; 